...
flowchart LR
    ...
    phonesim["phone-simulator<br/>Spring Boot 3 / Java 21"]:::svc
    sched["High-Performance Scheduler<br/>Go · HTTP + Kafka"]:::svc
    cap["CAP simulator<br/>GSM SIGTRAN"]:::ext

    redis[("Redis<br/>call:{testId}<br/>timer:duration:{testId}<br/>timer:no-answer:{testId}")]:::store

    ce["Kafka topic<br/><b>call-event-queue</b><br/>key = testId<br/>CallRecord: INITIAL / LAST_CHUNK"]:::topic
...
| phone-simulator | CAP simulator | Kafka `call-event-queue` | `CallRecordPayload` JSON, `callState ∈ {INITIAL, LAST_CHUNK}` |
...
sequenceDiagram
    ...
    P->>R: SETNX call:{testId}<br/>state = PENDING
    P->>R: state = DIALING
    P->>Kce: produce(key=testId, INITIAL)
    Kce->>C: deliver INITIAL
    P->>R: state = RINGING
    P->>S: POST /v1/timers<br/>(eventType=NO_ANSWER, delay=30s)
    P->>R: timer:no-answer:{testId}
    P-->>T: 202 Accepted {testId, state: RINGING}

    Note over C: SCP authorises call<br/>O_ANSWER sent
    C->>Kae: produce(key=testId, AnswerEvent)
    Kae->>P: consume AnswerEvent
    P->>S: DELETE /v1/timers/{noAnswerId}
    P->>R: state = ANSWERED
    P->>S: POST /v1/timers<br/>(eventType=RELEASE, delay=durationSeconds·1000)
    ...
    P->>Kce: produce(key=testId, LAST_CHUNK)
    Kce->>C: deliver LAST_CHUNK
    P->>R: state = RELEASED
...
- The no-answer guard is best-effort cancelled on answer; if the cancel races and the guard fires
  on an already-ANSWERED call, `handleNoAnswer` is idempotent and silently exits.
...
sequenceDiagram
    ...
    P->>R: SETNX call:{testId} (PENDING -> DIALING -> RINGING)
    P->>Kce: produce INITIAL
    Kce->>C: deliver INITIAL
    P->>S: POST /v1/timers (NO_ANSWER, delay=30s)
    P-->>T: 202 Accepted {testId, state: RINGING}

    Note over C,Kae: SCP rejects or never answers<br/>(no message produced to cap.answer-events.v1)

    Note over S: 30 s elapse, no AnswerEvent ever consumed
    S->>Ktt: produce FireEvent NO_ANSWER
    Ktt->>P: consume FireEvent
    P->>R: state = FAILED<br/>failureReason = no_answer_timeout
    P->>T: webhook POST CALL_FAILED
```

Threshold is `phonesim.call.no-answer-timeout` (default 30 s). On fire, phone-simulator does **not**
publish `LAST_CHUNK` — there's no answered call to release on the CAP side, and the existing
`SimulatorCapListener.onCAPReleaseCall` path already cancels CAP-side timers if the SCP issued
a release.
...
SMS is synchronous and skips the answer-event/timer paths entirely:
...
- One Kafka message on `call-event-queue` with `serviceKey=205`, `smsRecord=true`,
  `callState=INITIAL`. No follow-up `LAST_CHUNK`.
...
| Kind | Path |
|---|---|
| Voice (happy) | `PENDING → DIALING → RINGING → ANSWERED → RELEASED` |
| Voice (no answer) | `PENDING → DIALING → RINGING → FAILED` (`failureReason=no_answer_timeout`) |
| Voice (publish/scheduler error) | `PENDING/DIALING/RINGING → FAILED` |
| SMS | `PENDING → ANSWERED → RELEASED` (or `FAILED`) |
